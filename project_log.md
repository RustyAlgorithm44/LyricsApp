...
*   **Swipeable View Modes (ViewPager2 Integration):**
    *   Refactored `MainActivity` to use `ViewPager2`, enabling horizontal swipe gestures to navigate between "All Songs", "By Deity", "By Composer", and "By Category" views.
    *   Each view mode is now managed by its own `Fragment` (`AllSongsFragment`, `DeityListFragment`, `ComposerListFragment`, `CategoryListFragment`), encapsulating its specific UI and data observation logic.
